...
def auto_reply_email(request):
    if request.method == 'POST':
        try:
            ...
            # Generate AI-powered reply using the new function
            ai_reply = llm_util.generate_ai_reply(sender_name, receiver_name, inquiry_questions, paragraph)
            
            # Determine status based on what was extracted
            if sender_name and receiver_name:
                status = 'success'
            elif sender_name:
                status = 'Missing receiver name in the response'
            elif receiver_name:
                status = 'Missing sender name in the response'
            else:
                status = 'Missing sender and receiver name in the response'
